...
export type Salon = { id: string; nom: string; metier: Metier; plan: string };
...
export function SalonProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!su) { router.push("/onboarding"); return; }
      const { data: s } = await supabase.from("salons").select("id, nom, metier, plan").eq("id", su.salon_id).single();
