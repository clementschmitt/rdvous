...
export async function POST(req: NextRequest) {
  const { salon_id, prestation_id, date, heure, prenom, nom, email, telephone, adresse_domicile } = await req.json();

  if (!salon_id || !prestation_id || !date || !heure || !prenom || !nom || !email || !telephone) {
  ...
  await admin.from("rendez_vous_prestations").insert({ rendez_vous_id: rdv.id, prestation_id });
