...
export async function POST(req: NextRequest) {
  ...
  const { salon_id, prenom, nom, email, telephone, prestation_ids, date, website } = await req.json();
  ...
  try {
    ...
    await sendEmail({
      ...
      html: `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#222">
        ...
        <p style="font-size:14px;line-height:1.6">Si une place se libère ce jour-là, vous recevrez un message pour réserver en priorité. La réservation reste ouverte à toutes les personnes en attente : le premier qui réserve obtient le créneau.</p>
